...
```python
  sessions_spawn(
      agentId="podcast-translator",  # ← Явный ID
      task=f"...",
      runtime="subagent"
  )
...
python3 $SKILL_DIR/skills/podcast-translator/scripts/prepare_transcript.py \\
  "$TRANSCRIPT_FILE" \\
...
python3 $SKILL_DIR/skills/podcast-translator/scripts/extract_tts_text.py \\
...
python3 $SKILL_DIR/skills/podcast-translator/scripts/generate_tts.py \\
  "$SKILL_DIR/translations/{basename}_ru_tts.txt" \\
